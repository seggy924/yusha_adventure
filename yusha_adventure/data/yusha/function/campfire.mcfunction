...
#execute at @e[type=item,nbt={Item:{id:"minecraft:bone_meal"}}] unless entity @e[type=marker,tag=bone_meal,distance=..1] if block ~ ~ ~ campfire[lit=true] align xyz run summon item_display ~0.6 ~1 ~0.5 {Tags:["campfire"],transformation:{left_rotation:[0.2f,0f,1.8f,1f],right_rotation:[0f,0f,0f,1f],translation:[0f,0f,0f],scale:[1f,1f,1f]},item:{id:"netherite_sword",count:1}}
...
#알레이 소환
#execute at @e[type=item,nbt={Item:{id:"minecraft:bone_meal"}}] unless entity @e[type=marker,tag=bone_meal,distance=..1] if block ~ ~ ~ campfire[lit=true] align xyz run summon allay ~ ~1 ~3 {Invulnerable:1b,Tags:["campfire_mob"],DeathLootTable:""}
